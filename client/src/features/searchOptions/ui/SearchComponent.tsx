...
type SearchComponentRef = {
  clear: () => void;
};

const SearchComponent = forwardRef<SearchComponentProps, SearchComponentRef>(
  ({ onClear }, ref: ForwardedRef<SearchComponentRef>) => {
    ...
    // Экспортируем функцию clear через ref
              ...
              <div className="border-b border-[#D1E3F6]">
                ...
                <ul>
                  {filteredCategories.map((category) => (
                    <li
                      key={category.id}
                      className="hover:bg-[#D1E3F6] transition-all duration-300"
                    >
                      <button
                        type="button"
                        onClick={() => {
                          void navigate(`/categories/${category.id.toString()}`);
                          // handleClear(); // Добавляем вызов handleClear
                        }}
